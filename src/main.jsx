  ...
  return (
    <div className="min-h-screen text-slate-900 app-bg">
      <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <button onClick={() => setPage("home")} className="flex items-center gap-3 font-bold text-xl logo-text">
            ...
            <span className="logo-buu">Muhembo</span><span className="logo-pass">Bus</span>
          ...
          <div>
            {user ? (
              ...
              <button onClick={() => setPage("login")} className="bg-blue-700 text-white px-4 py-2 rounded-xl text-sm">Login</button>
            )}
          </div>
        </div>
      </nav>
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
          ...
          <section className="grid md:grid-cols-2 gap-8 items-center py-10">
            <div>
              ...
              <div className="flex flex-wrap gap-3">
                <button onClick={() => setPage("book")} className="bg-blue-700 hover:bg-blue-800 text-white px-6 py-3 rounded-2xl font-semibold">Book Now</button>
                <button onClick={() => setPage("routes")} className="bg-white border border-slate-300 px-6 py-3 rounded-2xl font-semibold">View Routes</button>
              </div>
            </div>
          ...
          <section className="max-w-md mx-auto bg-white rounded-3xl shadow p-6 border border-slate-200">
            ...
            <div className="space-y-4">
              ...
              <div className="password-field">
                <input className="input password-input" placeholder="Password" type={showPassword ? "text" : "password"} value={authForm.password} onChange={(e) => setAuthForm({ ...authForm, password: e.target.value })} />
              ...
              {authMode === "signup" ? (
                <button onClick={signup} className="w-full bg-green-600 text-white py-3 rounded-2xl font-semibold">✅ Create Passenger Account</button>
              ) : authMode === "forgot" ? (
                <>
                  <button onClick={resetPassword} className="w-full bg-amber-500 text-white py-3 rounded-2xl font-semibold">🔐 Reset Password</button>
                  <button type="button" onClick={() => setAuthMode("login")} className="w-full bg-slate-100 text-slate-700 py-3 rounded-2xl font-semibold">Back to Login</button>
                </>
              ) : (
                <>
                  <button onClick={() => login("user")} className="w-full bg-blue-700 text-white py-3 rounded-2xl font-semibold">👤 Login as Passenger</button>
                  <button onClick={() => login("admin")} className="w-full bg-slate-900 text-white py-3 rounded-2xl font-semibold">🛡️ Login as Admin</button>
                  <button type="button" onClick={() => setAuthMode("forgot")} className="forgot-link">Forgot password?</button>
        ...
        {page === "routes" && (
          <section>
            ...
            <div className="grid md:grid-cols-2 gap-5">
              {filteredRoutes.map((route) => <RouteCard key={route.id} route={route} onBook={() => { setBookingForm({ ...bookingForm, from: route.from, to: route.to, routeId: route.id }); setPage("book"); }} />)}
            </div>
          </section>
        )}
            ...
            <form onSubmit={makeBooking} className="lg:col-span-2 bg-white rounded-3xl shadow p-6 border border-slate-200">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <select className="input md:col-span-2" value={bookingForm.routeId} onChange={(e) => setBookingForm({ ...bookingForm, routeId: e.target.value, seatNo: "" })} disabled={!bookingForm.from || !bookingForm.to}>
                  ...
                  {matchingRoutes.map((route) => <option key={route.id} value={route.id}>{route.bus} - {route.time} - KES {route.price}</option>)}
                </select>
                  ...
                  {!selectedRoute || !bookingForm.travelDate ? (
                    ...
                    <div className="max-w-sm mx-auto">
                      <div className="flex justify-end mb-3">
                        <div className="w-16 h-12 rounded-xl bg-red-600 text-white flex items-center justify-center font-bold">Dr.</div>
                      </div>
              ...
              <button className="mt-6 bg-blue-700 hover:bg-blue-800 text-white px-6 py-3 rounded-2xl font-semibold">Confirm Booking & Pay</button>
            </form>
        ...
        {page === "my-bookings" && (
          <section>
            ...
            {myBookings.length === 0 ? (
              <div className="bg-white rounded-3xl p-8 border border-slate-200 text-center text-slate-600">No bookings found.</div>
            ) : (
              <div className="grid md:grid-cols-2 gap-5">
                {myBookings.map((booking) => <BookingCard key={booking.id} booking={booking} isAdmin={user?.role === "admin"} onDelete={() => deleteBooking(booking.id)} />)}
              </div>
            )}
          </section>
        )}
          ...
          <section className="grid lg:grid-cols-3 gap-6">
            <form onSubmit={addRoute} className="bg-white rounded-3xl shadow p-6 border border-slate-200 h-fit">
              ...
              <div className="space-y-3">
                ...
                <button className="w-full bg-blue-700 text-white py-3 rounded-2xl font-semibold">➕ Add Route</button>
              </div>
            </form>
                ...
                <div className="space-y-3">
                  {bookings.slice(0, 5).map((booking) => (
                    <div key={booking.id} className="flex items-center justify-between border-b border-slate-100 pb-3">
                      ...
                      <button onClick={() => deleteBooking(booking.id)} className="text-red-600">🗑️</button>
                    </div>
                  ))}
      ...
      <footer className="footer-section">
        <div className="footer-grid">
          <div>
            ...
            <p>
              Muhembo Bus Service provides travelers with reliable, comfortable, and convenient online bus booking.
            </p>
            <p className="footer-slogan">Travel Smart, Travel Muhembo</p>
          </div>

          <div>
            <div className="footer-logo"><span className="logo-buu">Muhembo</span><span className="logo-pass">Bus</span></div>
      ...
      <style>{`
        ...
        .app-bg { background: radial-gradient(circle at top left, rgba(255, 183, 77, 0.3), transparent 30%), radial-gradient(circle at bottom right, rgba(236, 72, 153, 0.35), transparent 35%), linear-gradient(135deg, #f97316 0%, #ec4899 45%, #6366f1 100%); min-height: 100vh; }
        .app-bg::before { content: ""; position: fixed; inset: 0; pointer-events: none; background-image: linear-gradient(rgba(255,255,255,0.08) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.08) 1px, transparent 1px); background-size: 42px 42px; mask-image: linear-gradient(to bottom, rgba(0,0,0,0.8), transparent); }
        button { cursor: pointer; transition: 0.2s ease; }
        button:hover { transform: translateY(-1px); }
        nav { box-shadow: 0 8px 24px rgba(15, 23, 42, 0.18); }
        main { position: relative; z-index: 1; }
        h1, main h2 { color: #ffffff; text-shadow: 0 3px 14px rgba(15, 23, 42, 0.3); }
        form h2, form h3, .bg-white h2, .bg-white h3 { color: #0f172a; text-shadow: none; }
        .input { width: 100%; border: 1px solid #cbd5e1; border-radius: 1rem; padding: 0.8rem 1rem; outline: none; background: #ffffff; color: #0f172a; transition: 0.2s ease; }
        .input:focus { border-color: #1d4ed8; box-shadow: 0 0 0 4px rgba(29, 78, 216, 0.12); }
        .auth-tabs { display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem; background: #f1f5f9; padding: 0.4rem; border-radius: 1rem; margin-bottom: 1rem; }
        .three-tabs { grid-template-columns: 1fr 1fr 1fr; }
        .auth-tabs button { border: none; border-radius: 0.8rem; padding: 0.75rem; font-weight: 800; color: #475569; }
        .active-auth-tab { background: #ffffff; color: #1d4ed8 !important; box-shadow: 0 6px 18px rgba(15, 23, 42, 0.08); }
        .forgot-link { border: none; background: transparent; color: #2563eb; font-weight: 800; text-align: center; padding: 0.25rem; }
        .forgot-link:hover { transform: none; text-decoration: underline; }
        .admin-hint { background: #f8fafc; border: 1px dashed #94a3b8; color: #475569; font-size: 0.85rem; padding: 0.75rem; border-radius: 1rem; text-align: center; }
        .success-popup { position: fixed; top: 90px; right: 24px; z-index: 2000; background: #16a34a; color: #ffffff; padding: 1rem 1.2rem; border-radius: 1rem; display: flex; align-items: center; gap: 0.6rem; box-shadow: 0 18px 40px rgba(0,0,0,0.25); animation: slideIn 0.3s ease; }
        .password-field { position: relative; }
        .password-input { padding-right: 3.4rem; }
        .eye-button { position: absolute; right: 0.6rem; top: 50%; transform: translateY(-50%); border: none; background: #f1f5f9; width: 2.4rem; height: 2.4rem; border-radius: 0.8rem; display: flex; align-items: center; justify-content: center; font-size: 1rem; }
        .eye-button:hover { transform: translateY(-50%) scale(1.03); background: #e2e8f0; }
        section, form, .rounded-3xl { animation: fadeIn 0.35s ease-in-out; }
        .shadow, .shadow-lg { box-shadow: 0 20px 55px rgba(15, 23, 42, 0.22); }
        .bg-white { background: rgba(255, 255, 255, 0.94); backdrop-filter: blur(12px); }
        main > section > div > p, main > section > p { color: rgba(255,255,255,0.86) !important; }
        .footer-section { position: relative; z-index: 1; margin-top: 4rem; background: #303942; color: #ffffff; padding: 4rem 2rem; }
        .footer-grid { max-width: 1100px; margin: 0 auto; display: grid; grid-template-columns: repeat(3, 1fr); gap: 3rem; }
        .logo-icon { width: 36px; height: 36px; border-radius: 10px; display: inline-flex; align-items: center; justify-content: center; background: linear-gradient(135deg, #2563eb, #22c55e); color: #fff; box-shadow: 0 6px 16px rgba(0,0,0,0.2); font-size: 18px; }
        .logo-text { letter-spacing: -0.5px; position: relative; }
        .nav-slogan { display: block; font-size: 0.72rem; font-weight: 700; color: #475569; margin-left: 0.35rem; letter-spacing: 0; }
        .logo-buu { color: #22c55e; font-weight: 900; letter-spacing: -0.3px; }
        .logo-pass { color: #2563eb; font-weight: 900; margin-left: -0.15rem; }
        .footer-logo { font-size: 2.7rem; font-weight: 900; margin-bottom: 1rem; letter-spacing: -1.5px; }
        .social-icons { display: flex; align-items: center; gap: 0.7rem; font-size: 1.55rem; color: #db2777; font-weight: 900; }
        .social-icons span { display: inline-flex; align-items: center; justify-content: center; width: 28px; height: 28px; }
        .footer-slogan { margin-top: 1rem; color: #fbbf24 !important; font-weight: 800; }
        .footer-section h3 { font-size: 1.4rem; font-weight: 800; margin-bottom: 1rem; }
        .footer-section p { color: rgba(255,255,255,0.9); line-height: 1.7; font-size: 1rem; }
        .chat-widget { position: fixed; bottom: 20px; left: 20px; display: flex; align-items: center; gap: 10px; z-index: 1000; }
        .chat-icon { width: 60px; height: 60px; border-radius: 50%; background: #f97316; color: white; display: flex; align-items: center; justify-content: center; font-size: 24px; box-shadow: 0 10px 25px rgba(0,0,0,0.2); }
        .chat-bubble { background: white; border-radius: 12px; padding: 10px 14px; box-shadow: 0 10px 25px rgba(0,0,0,0.15); font-size: 14px; }
        .chat-bubble strong { display: block; color: #111827; }
        .chat-bubble p { margin: 0; color: #6b7280; font-size: 13px; }
...
function InfoCard({ label, value }) {
  return <div className="bg-white/10 rounded-2xl p-3"><p className="text-2xl font-bold">{value}</p><p className="text-xs text-blue-100">{label}</p></div>;
}
...
function ActionCard({ icon, title, text, onClick }) {
  return (
    <button onClick={onClick} className="text-left bg-white rounded-3xl shadow p-6 border border-slate-200 hover:shadow-lg transition">
      <div className="w-12 h-12 bg-blue-100 text-blue-700 rounded-2xl flex items-center justify-center mb-4 text-2xl">{icon}</div>
  ...
  return (
    <div className="bg-white rounded-3xl shadow p-6 border border-slate-200">
      <div className="flex items-start justify-between gap-4 mb-4">
        ...
        <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-sm font-semibold">KES {route.price}</span>
      </div>
      <div className="grid grid-cols-2 gap-3 text-sm mb-5">
        <p>📅 {route.time}</p>
        <p>📍 {route.seats} seats</p>
      </div>
      <button onClick={onBook} className="w-full bg-blue-700 text-white py-3 rounded-2xl font-semibold">Book This Route</button>
    </div>
  );
}
  ...
  return (
    <div className="bg-white rounded-3xl shadow p-6 border border-slate-200">
      <div className="flex items-start justify-between gap-4 mb-4">
        ...
        <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-sm font-semibold">{booking.status}</span>
      </div>
      ...
      {isAdmin && <button onClick={onDelete} className="mt-5 text-red-600 font-semibold">🗑️ Delete Booking</button>}
    </div>
  );
}

function Stat({ title, value }) {
  return <div className="bg-white rounded-3xl shadow p-5 border border-slate-200"><p className="text-slate-500 text-sm">{title}</p><p className="text-2xl font-bold">{value}</p></div>;
}
